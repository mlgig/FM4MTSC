"""
Data loading utilities for deep learning time series classification.
"""
...
def load_npy_dataset(
    ...
    """
    Load and prepare numpy dataset for deep learning time series classification.
...
def get_dataset_info(file_path: str) -> Dict[str, Union[str, int, tuple]]:
    ...
    info = {
        ...
        "time_series_length": data["train"]["X"].shape[1],
        "num_dimensions": 1
        if len(data["train"]["X"].shape) == 2
        else data["train"]["X"].shape[2],
...
def prepare_data_for_dl(X_train, X_test):
    """
    Prepare data specifically for deep learning models.
    Ensures correct dimensionality and normalization if needed.
    ...
    """
    # Ensure 3D input for deep learning models (samples, timesteps, features)
    if len(X_train.shape) == 2:
        # Add feature dimension if only (samples, timesteps)
        X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)
        X_test = X_test.reshape(X_test.shape[0], X_test.shape[1], 1)

    # Simple normalization if needed
    # X_train = (X_train - X_train.mean(axis=(0, 1), keepdims=True)) / (X_train.std(axis=(0, 1), keepdims=True) + 1e-8)
    # X_test = (X_test - X_test.mean(axis=(0, 1), keepdims=True)) / (X_test.std(axis=(0, 1), keepdims=True) + 1e-8)
