...
def validate(
    ...
    if condition:
        ...
        # Debugging: Print the structure of per_batch
        print("Debugging per_batch contents:")
        for key, value in per_batch.items():
            print(f"{key}: type={type(value)}, shape={getattr(value, 'shape', 'N/A')}")

        # Convert all elements to numpy arrays with dtype=object to handle varying shapes
        for key in per_batch:
            per_batch[key] = np.asarray(per_batch[key], dtype=object)

        # Saving the predictions
        try:
            np.savez(pred_filepath, **per_batch)
        except Exception as e:
            print(f"Error while saving the predictions: {e}")
            # Optionally, save them separately if you can't save as a .npz
            for key, value in per_batch.items():
                np.save(f"{pred_filepath}_{key}.npy", value)
